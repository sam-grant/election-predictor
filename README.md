...
- **Interaction terms drove performance**: The jump from Selected (10 features) to Interact (15 features with interactions) provided the largest gains (+23% for RF/SVC, +17% for LR)

- **Minimal hyperparameter sensitivity**: Random Forest and SVC achieved 88.2% accuracy with default hyperparameters; grid search found no improvements, suggesting these models were insensitive to hyperparameter choices on this dataset

- **Overfitting concerns**: Extensive hyperparameter searches (216-22,032 combinations) typically yielded minimal or no improvements, suggesting the models (except XGBoost) are not overfitting to hyperparameter selection

- **XGBoost required tuning but underperformed**: The more complex XGBoost architecture benefited most from hyperparameter optimisation but still underperformed compared with simpler models, suggesting overfitting on limited data rather than capturing genuine signal

- **Model convergence**: **Three independent algorithms (LR, RF, SVC) converged on identical 88.2% accuracy** after optimisation, suggesting 88.2% represents the performance ceiling achievable with macro socioeconomic indicators alone

This analysis demonstrates that **model simplicity combined with feature engineering outperformed complex architectures**, strengthening confidence in the 88.2% result as genuine rather than an artifact of overfitting.
